...
export class BaseProcessor {
    ...
    protected createWeeklyMetrics(dailyMetrics: DailyMetrics[]): WeeklyMetrics[] {
        ...
        const sortedData = dailyMetrics.sort((a, b) => a.date.localeCompare(b.date));
     ...
     */
    protected saveWeeklyFiles(weeklyMetrics: WeeklyMetrics[], suffix: string): void {
        this.ensureResultsDir();
        
        weeklyMetrics.forEach(weekResult => {
            const outputFile = path.join(this.resultsDir, `${weekResult.week}-${suffix}.json`);
            fs.writeFileSync(outputFile, JSON.stringify(weekResult, null, 2));
            console.log(`${suffix.charAt(0).toUpperCase() + suffix.slice(1)} results written to ${outputFile}`);
        });
    }
    ...
    protected findFilesByExtension(extension: string, prefix?: string): string[] {
        try {
            ...
            if (filteredFiles.length === 0) {
                ...
                throw new Error(`No ${extension} files${prefixText} found in ${this.sourcesDir}`);
            }
